...
function deploy() {
    ...
    echo "Package Directory contains ..."
    ls ${PKG_DIR}/*.{deb,rpm}
...
function get_versions_url() {
    ...
    PATH="/api/v1/repos/${PACKAGECLOUD_ORGANIZATION}/${PACKAGECLOUD_REPO}/packages/${PKG_TYPE}/${PKG_OS_NAME}/${PKG_OS_VERSION}/${PKG_NAME}.json"
    curl -Ss -q https://${PACKAGECLOUD_TOKEN}:@"${HOST}${PATH}" | jq -r .[0].versions_url
}

# TODO: Check if CURL response code was successful
function get_revision() {
    versions_url="$(get_versions_url)?per_page=1000"
