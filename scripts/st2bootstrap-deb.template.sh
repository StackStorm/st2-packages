...
USERNAME=''
PASSWORD=''
DISTRIB=`lsb_release -is 2>/dev/null | tr [:upper:] [:lower:]`
SUBTYPE=`lsb_release -cs 2>/dev/null | tr [:upper:] [:lower:]`

case "${DISTRIB}${SUBTYPE}" in
    'ubuntutrusty'|'ubuntuxenial'|'ubuntubionic'|'debianbuster')
        # No-op, these OS' are supported.
        ;;
    *)
        cat <<EOF
${DISTRIB}/${SUBTYPE} is an unsupported distribution. Supported distributions are
 - Ubuntu
    - 14.04(trusty)
    - 16.04(xenial)
    - 18.04(bionic)
...
install_mongodb() {
    ...
    case "$SUBTYPE" in
        'bionic')
            ...
            ;;
        'buster')
            # Mongo only provides 4.2 packages for Debian 10
            wget -qO - https://www.mongodb.org/static/pgp/server-4.2.asc | sudo apt-key add -
            echo "deb http://repo.mongodb.org/apt/debian ${SUBTYPE}/mongodb-org/4.2 main" | sudo tee /etc/apt/sources.list.d/mongodb-org-4.2.list
            ;;
        *)
...
get_full_pkg_versions() {
    if [ "$VERSION" != '' ];
    then
        local ST2_VER=$(apt-cache show st2 | awk '/Version/ {print $2}' | grep ^${VERSION//./\\.} | sort --version-sort | tail -n 1)
        if [ -z "$ST2_VER" ]; then
        ...
        case "$SUBTYPE" in
            'bionic'|'buster')
                # Bionic/Buster doesn't support Mistral
                local ST2MISTRAL_VER="none"
                ;;
            *)
                local ST2MISTRAL_VER=$(apt-cache show st2mistral | awk '/Version/ {print $2}' | grep ^${VERSION//./\\.} | sort --version-sort | tail -n 1)
        ...
        local ST2WEB_VER=$(apt-cache show st2web | awk '/Version/ {print $2}' | grep ^${VERSION//./\\.} | sort --version-sort | tail -n 1)
        ...
        local ST2CHATOPS_VER=$(apt-cache show st2chatops | awk '/Version/ {print $2}' | grep ^${VERSION//./\\.} | sort --version-sort | tail -n 1)
