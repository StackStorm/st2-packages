...
verify_dependency_status() {
# Check the service status of all the dependencies

count=0
while true; do
    rabbit_status=$(service rabbitmq-server status | grep "active (running)")
    mongo_status=$(service mongod status | grep "active (running)")
    if [[ $rabbit_status == *"Active"* && $mongo_status == *"Active"* ]]
    then
        echo "Rabbit and Mongodb Successfully Installed."
        echo "It took ${count} seconds."
        break
    else
        echo "Sleeping for a sec. Mongo and Rabbit are not active yet."
        sleep 1
        ((count+=1))
    fi
done
...
verify_st2() {
  ...
  timeout 60s st2 run core.local -- date -R
...
STEP="Verify st2 dependency status" && verify_dependency_status
